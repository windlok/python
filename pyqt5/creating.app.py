...
#QApplication, QWidget, QMainWindow ve QToolTip sınıflarını PyQt5.
#QtWidgets modülünden içe aktarır. Bu sınıflar, PyQt5 uygulamalarında sıklıkla kullanılan temel bileşenlerdir.
from PyQt5.QtGui import QIcon

def window():
    #app bu bir QApplication nesnesi oluşturur. 
    #QApplication, PyQt5 uygulamasının temelini oluşturur ve uygulamanın çalışması için gereklidir.
    app = QApplication(sys.argv)
    #QWidget, PyQt5'te temel bir pencere sınıfıdır. 
    #QWidget, diğer tüm kullanıcı arayüzü öğelerinin temelini oluşturur ve kendi başına bir pencere olarak kullanılabilir.
    win = QWidget()
    #QMainWindow, PyQt5'te daha gelişmiş bir pencere sınıfıdır. 
    #QMainWindow, menü çubuğu, araç çubuğu, durum çubuğu gibi özelliklere sahip bir ana pencere sağlar.
    win=QMainWindow()
    ...
    #show() metodu, pencereyi ekranda görünür hale getirir. 
    #Bu metodu çağırmadan önce pencere oluşturulmuş olsa da, kullanıcıya gösterilmez.
    
    #setWindowIcon() metodu, pencerenin simgesini belirlemek için kullanılır. 
    #Bu metodun parametresi, simge olarak kullanılacak bir QIcon nesnesidir.
    ...
    #win.show() metodu, pencereyi ekranda görünür hale getirir.
    win.show()
    #sys.exit(app.exec_()) ifadesi, uygulamanın ana döngüsünü başlatır ve uygulama kapatıldığında programın düzgün bir şekilde sonlanmasını sağlar.
    sys.exit(app.exec_())
